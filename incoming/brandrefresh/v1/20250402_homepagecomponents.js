(function () {
  ...
  var CONTAINER_ID = "abv2-open-now-grid";
  ...
  function buildPill(type, text) {
    return '<span class="abv2-open-card__pill abv2-open-card__pill--' + type + '">' + escapeHtml(text) + "</span>";
  }
  ...
  function buildCard(record) {
    ...
    return (
      '<article class="abv2-open-card">' +
        '<a class="abv2-open-card__link" href="' + escapeHtml(href) + '" aria-label="Go to ' + escapeHtml(name) + '">' + escapeHtml(name) + "</a>" +
        '<div class="abv2-open-card__media" style="--card-bg:' + escapeHtml(bg) + ';">' +
          (logo ? '<img class="abv2-open-card__logo" src="' + escapeHtml(logo) + '" alt="' + escapeHtml(name) + ' logo">' : "") +
        "</div>" +
        '<div class="abv2-open-card__body">' +
          '<div class="abv2-open-card__top">' +
            '<div class="abv2-open-card__kicker">Open now</div>' +
            '<h3 class="abv2-open-card__title">' + escapeHtml(name) + "</h3>" +
            '<p class="abv2-open-card__desc">' + escapeHtml(desc) + "</p>" +
          "</div>" +
          '<div class="abv2-open-card__bottom">' +
            (pills.length ? '<div class="abv2-open-card__meta">' + pills.join("") + "</div>" : "") +
            '<div class="abv2-open-card__cta">View investment</div>' +
          "</div>" +
        "</div>" +
      "</article>"
    );
  ...
  function init() {
    ...
    container.innerHTML = '<div class="abv2-open-now__loading">Loading open municipal investments...</div>';
    ...
    fetch(ENDPOINT, { cache: "no-store" })
      ...
      .then(function (data) {
        ...
        container.innerHTML = open.length
          ...
          : '<div class="abv2-open-now__empty">There are no open municipal investments at the moment. Please check again later.</div>';
      })
      .catch(function () {
        container.innerHTML = '<div class="abv2-open-now__error">Sorry — we couldn’t load municipal investments right now.</div>';
      });
  }
